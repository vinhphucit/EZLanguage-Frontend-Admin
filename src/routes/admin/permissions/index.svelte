<script lang="ts">
	...
	export let data: PaginableList<Permission>;
	...
	async function onGetPageClick(page = 0) {
		const res = await getPermissions(page * itemPerPage, itemPerPage);
		if (!res.error) {
			data = res.data;
			currentPage = page;
	...
	async function onDeletePermissionClick(id: string) {
...
<main class="h-full pb-16 overflow-y-auto">
	<div class="container grid px-6 mx-auto">
		<div class="flex justify-between items-center mt-6">
			<h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Permissions</h2>
			<a
									...
									<td class="py-3 w-auto">
										<div class="flex justify-between">
											...
											<button
												class="px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg focus:outline-none focus:shadow-outline-gray"
												on:click={() => {
													onDeletePermissionClick(prop.id);
												}}
												aria-label="Delete"
											>
												<svg
