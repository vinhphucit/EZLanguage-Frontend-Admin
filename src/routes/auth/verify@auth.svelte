<script lang="ts">
	...
	import { signIn, verify } from '$lib/network/api/auth-api';
					...
					<div class="w-full">
						...
						<div>
							...
							<label class="block text-sm">
								...
								<input name="code" type="text" placeholder="ActivationCode" bind:value={code} />
							</label>
							<Error errors={[errorMessage]} />
							<!-- You should use a button here, as the anchor is only used for the example  -->
							<button class="normal" on:click={onVerifyClick}>Verify</button>
						</div>
					</div>
				</div>
			</div>
