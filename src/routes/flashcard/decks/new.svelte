<script lang="ts">
	...
	import { createDeck } from '$lib/network/api/deck-api';

	export let name = '',
		code = '';
	let errorMessage = '';
	async function onCreateClick() {
		const res = await createDeck(name, code);
		if (!res.error) {
			goto(`/flashcard/decks`);
		} else {
...
<div class="container  max-w-3xl py-20 mx-auto">
	<h2 class="mt-4 mb-4 text-3xl font-extrabold text-gray-900">Create New Deck</h2>
	<div>
		<label for="">
			Title
