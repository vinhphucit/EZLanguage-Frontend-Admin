<script lang="ts">
	...
	import { deleteRole, getRoles } from '$lib/network/api/role-api';
	import { getDecks } from '$lib/network/api/deck-api';
	import type { Deck } from '$lib/network/models/DeckResponse';

	export let loaderStatus: LoadingStatus = LoadingStatus.LOADING;
	export let data: PaginableList<Deck>;
	...
	async function onGetPageClick(page = 0) {
		const res = await getDecks(page * itemPerPage, itemPerPage);
		if (!res.error) {
			data = res.data;
			currentPage = page;
...
<main class="h-full pb-16 overflow-y-auto">
	<div class="container grid px-6 mx-auto">
		<div class="flex justify-between items-center mt-6">
			<h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Decks</h2>
			<a
